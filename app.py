...
def auth_user():
    ...
    existing = supabase.table('users').select('*').eq('telegram_id', telegram_id).maybe_single().execute()
...
def get_user(telegram_id: str):
    try:
        user_result = supabase.table('users').select('*').eq('telegram_id', str(telegram_id)).maybe_single().execute()
